...
/// and an [Pic] that stores the font [map].
BitmapFont readFont(String font, Pic map) => BitmapFont.fromFnt(font, map);
...
class BitmapFont {
  ...
  /// Decode a [BitmapFont] from the contents of [font] definition (.fnt) file,
  /// and an [Pic] that stores the font [map].
  BitmapFont.fromFnt(String fnt, Pic page) {
    Map<int, Pic> fontPages = {0: page};
  ...
  void _parseFnt(XmlDocument xml, Map<int, Pic> fontPages, [Archive arc]) {
      ...
      if (name == 'info') {
        ...
        for (var page in _childElements(c)) {
          ...
          if (arc != null) {
            ...
            Pic image = PngDecoder().decodeImage(imageFile.content as List<int>);
    ...
    for (var c in _childElements(font)) {
      ...
      if (name == 'chars') {
        for (var char in _childElements(c)) {
          ...
          Pic fontImage = fontPages[page];

          BitmapFontCharacter ch = BitmapFontCharacter(id, width, height, xoffset, yoffset, xadvance, page, chnl);
...
class BitmapFontCharacter {
  ...
  BitmapFontCharacter(
      this.id, int width, int height, this.xoffset, this.yoffset, this.xadvance, this.page, this.channel)
      : this.width = width,
        this.height = height,
