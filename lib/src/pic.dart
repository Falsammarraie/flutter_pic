...
class Pic {
  ...
  Pic(this.width, this.height, {this.channels = Channels.rgba, ExifData exif, ICCProfileData iccp})
      : this.data = Uint32List(width * height),
        this.exif = ExifData.from(exif),
        this.iccProfile = iccp;

  Pic.rgb(this.width, this.height, {ExifData exif, ICCProfileData iccp})
  ...
  ///                             format: Format.rgba);
  Pic.fromBytes(int width, int height, List<int> bytes,
      {ExifData exif, ICCProfileData iccp, Format format = Format.rgba, this.channels = Channels.rgba})
      : this.width = width,
        this.height = height,
  ...
  /// coordinates.
  int getPixelInterpolate(num fx, num fy, [Interpolation interpolation = Interpolation.linear]) {
  ...
  int getPixelCubic(num fx, num fy) {
    ...
    num Ip1 = _cubic(dx, getGreen(Ipp), getGreen(Icp), getGreen(Inp), getGreen(Iap));
    num Ip2 = _cubic(dx, getBlue(Ipp), getBlue(Icp), getBlue(Inp), getBlue(Iap));
    num Ip3 = _cubic(dx, getAlpha(Ipp), getAlpha(Icp), getAlpha(Inp), getAlpha(Iap));
    ...
    num Ic1 = _cubic(dx, getGreen(Ipc), getGreen(Icc), getGreen(Inc), getGreen(Iac));
    num Ic2 = _cubic(dx, getBlue(Ipc), getBlue(Icc), getBlue(Inc), getBlue(Iac));
    num Ic3 = _cubic(dx, getAlpha(Ipc), getAlpha(Icc), getAlpha(Inc), getAlpha(Iac));
    ...
    num In1 = _cubic(dx, getGreen(Ipn), getGreen(Icn), getGreen(Inn), getGreen(Ian));
    num In2 = _cubic(dx, getBlue(Ipn), getBlue(Icn), getBlue(Inn), getBlue(Ian));
    num In3 = _cubic(dx, getAlpha(Ipn), getAlpha(Icn), getAlpha(Inn), getAlpha(Ian));
    ...
    num Ia1 = _cubic(dx, getGreen(Ipa), getGreen(Ica), getGreen(Ina), getGreen(Iaa));
    num Ia2 = _cubic(dx, getBlue(Ipa), getBlue(Ica), getBlue(Ina), getBlue(Iaa));
    num Ia3 = _cubic(dx, getAlpha(Ipa), getAlpha(Ica), getAlpha(Ina), getAlpha(Iaa));
  ...
  static Uint32List _convertData(int width, int height, List<int> bytes, Format format) {
    if (format == Format.rgba) {
      return bytes is Uint32List ? Uint32List.fromList(bytes) : Uint32List.view(Uint8List.fromList(bytes).buffer);
    }

    List<int> input = bytes is Uint32List ? Uint8List.view(bytes.buffer) : bytes;
