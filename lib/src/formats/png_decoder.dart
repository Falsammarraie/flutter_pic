...
class PngDecoder extends Decoder {
    ...
    while (true) {
      ...
      switch (chunkType) {
        case 'IHDR':
          ...
          if (![GRAYSCALE, RGB, INDEXED, GRAYSCALE_ALPHA, RGBA].contains(_info.colorType)) {
            return null;
          }
        ...
        case 'bKGD':
          if (_info.colorType == 3) {
            ...
            _info.backgroundColor = ColorAdvanced.fromRgb(r, g, b);
  ...
  Pic decodeFrame(int frame) {
    ...
    Channels channels;
    if (_info.colorType == GRAYSCALE_ALPHA || _info.colorType == RGBA || _info.transparency != null) {
      channels = Channels.rgba;
    } else {
      channels = Channels.rgb;
    }

    Pic image = Pic(width, height, channels: channels);
    ...
    image.iccProfile = ICCProfileData(_info.iCCPName, ICCPCompression.deflate, _info.iCCPData);

    return image;
  }

  Pic decodeImage(List<int> data, {int frame = 0}) {
  ...
  Animation decodeAnimation(List<int> data) {
    ...
    if (!_info.isAnimated) {
      Pic image = decodeFrame(0);
      anim.addFrame(image);
      return anim;
    }

    int dispose = PngFrame.APNG_DISPOSE_OP_BACKGROUND;
    Pic lastImage = Pic(_info.width, _info.height);
    for (int i = 0; i < _info.numFrames; ++i) {
      //_frame = i;
      if (lastImage == null) {
        lastImage = Pic(_info.width, _info.height);
      } else {
        lastImage = Pic.from(lastImage);
      }

      PngFrame frame = _info.frames[i];
      Pic image = decodeFrame(i);
      if (image == null) {
        continue;
      }

      if (lastImage != null) {
        if (dispose == PngFrame.APNG_DISPOSE_OP_BACKGROUND || dispose == PngFrame.APNG_DISPOSE_OP_PREVIOUS) {
          lastImage.fill(_info.backgroundColor);
        }
        copyInto(lastImage, image,
            dstX: frame.xOffset, dstY: frame.yOffset, blend: frame.blend == PngFrame.APNG_BLEND_OP_OVER);
      } else {
        lastImage = image;
      }
  ...
  void _processPass(
      InputBuffer input, Pic image, int xOffset, int yOffset, int xStep, int yStep, int passWidth, int passHeight) {
    final int channels =
        (_info.colorType == GRAYSCALE_ALPHA) ? 2 : (_info.colorType == RGB) ? 3 : (_info.colorType == RGBA) ? 4 : 1;
    ...
    //int pi = 0;
    for (int srcY = 0, dstY = yOffset, ri = 0; srcY < passHeight; ++srcY, dstY += yStep, ri = 1 - ri, _progressY++) {
      ...
      for (int srcX = 0, dstX = xOffset; srcX < passWidth; ++srcX, dstX += xStep) {
  ...
  void _process(InputBuffer input, Pic image) {
    final int channels =
        (_info.colorType == GRAYSCALE_ALPHA) ? 2 : (_info.colorType == RGB) ? 3 : (_info.colorType == RGBA) ? 4 : 1;
  ...
  int _readBits(InputBuffer input, int numBits) {
    ...
    int mask = (numBits == 1)
        ? 1
        : (numBits == 2) ? 3 : (numBits == 4) ? 0xf : (numBits == 8) ? 0xff : (numBits == 16) ? 0xffff : 0;
    ...
    switch (_info.colorType) {
      case GRAYSCALE:
        ...
        if (_info.transparency != null) {
          int a = ((_info.transparency[0] & 0xff) << 24) | (_info.transparency[1] & 0xff);
          if (raw[0] == a) {
      ...
      case RGB:
        ...
        if (_info.transparency != null) {
          int tr = ((_info.transparency[0] & 0xff) << 8) | (_info.transparency[1] & 0xff);
          int tg = ((_info.transparency[2] & 0xff) << 8) | (_info.transparency[3] & 0xff);
          int tb = ((_info.transparency[4] & 0xff) << 8) | (_info.transparency[5] & 0xff);
      ...
      case INDEXED:
        ...
        int a = _info.transparency != null && raw[0] < _info.transparency.length ? _info.transparency[raw[0]] : 255;
