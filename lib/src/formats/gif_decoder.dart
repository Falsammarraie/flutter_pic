...
class GifDecoder extends Decoder {
  ...
  Animation decodeAnimation(List<int> bytes) {
    ...
    Pic lastImage = Pic(info.width, info.height);
    for (int i = 0; i < info.numFrames; ++i) {
      //_frame = i;
      if (lastImage == null) {
        lastImage = Pic(info.width, info.height);
      } else {
  ...
  Pic _decodeImage(GifImageDesc gifImage) {
    if (_buffer == null) {
      _initDecode();
    }
  ...
  void _updateImage(Pic image, int y, GifColorMap colorMap, Uint8List line) {
    if (colorMap != null) {
