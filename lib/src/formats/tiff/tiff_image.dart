...
class TiffImage {
  ...
  TiffImage(InputBuffer p) {
    ...
    if (width == null || height == null || bitsPerSample == null || compression == null) {
      return;
    }
    ...
    switch (photometricType) {
      ...
      case 2: // RGB
        ...
        break;
      case 3: // RGB Palette
        if (samplesPerPixel == 1 && (bitsPerSample == 4 || bitsPerSample == 8 || bitsPerSample == 16)) {
      ...
      case 4: // Transparency mask
        ...
        break;
      case 6: // YCbCr
        if (compression == COMPRESSION_JPEG && bitsPerSample == 8 && samplesPerPixel == 3) {
          imageType = TYPE_RGB;
        } else {
  ...
  bool get isValid =>
      width != null && height != null && samplesPerPixel != null && bitsPerSample != null && compression != null;

  Pic decode(InputBuffer p) {
  ...
  void _decodeTile(InputBuffer p, int tileX, int tileY) {
    ...
    if (bitsPerSample == 8 || bitsPerSample == 16) {
      if (compression == COMPRESSION_NONE) {
        ...
        try {
          decoder.decode(new InputBuffer.from(p, offset: 0, length: byteCount), bdata.buffer);
        } catch (e) {
          print(e);
        }
        ...
        if (predictor == 2) {
          ...
          for (int j = 0; j < tileHeight; j++) {
            ...
            for (int i = samplesPerPixel, len = tileWidth * samplesPerPixel; i < len; i++) {
        ...
        List<int> data = p.toList(0, byteCount);
        Pic tile = JpegDecoder().decodeImage(data);
          ...
          if (samplesPerPixel == 1) {
            ...
            if (image != null) {
              ...
              if (photometricType == 3 && colorMap != null) {
                c = getColor(
                    colorMap[colorMapRed + gray], colorMap[colorMapGreen + gray], colorMap[colorMapBlue + gray]);
              } else {
  ...
  void _jpegToImage(Pic tile, Pic image, int outX, int outY, int tileWidth, int tileHeight) {
    int width = tileWidth;
  ...
  void _decodeBilevelTile(InputBuffer p, int tileX, int tileY) {
    ...
    if (compression == COMPRESSION_PACKBITS) {
      ...
      bdata = InputBuffer(new Uint8List(tileWidth * tileHeight));
      try {
        new TiffFaxDecoder(fillOrder, tileWidth, tileHeight).decode1D(bdata, p, 0, tileHeight);
      } catch (_) {}
    } else if (compression == COMPRESSION_CCITT_FAX3) {
      bdata = InputBuffer(new Uint8List(tileWidth * tileHeight));
      try {
        new TiffFaxDecoder(fillOrder, tileWidth, tileHeight).decode2D(bdata, p, 0, tileHeight, t4Options);
      } catch (_) {}
    } else if (compression == COMPRESSION_CCITT_FAX4) {
      bdata = InputBuffer(new Uint8List(tileWidth * tileHeight));
      try {
        new TiffFaxDecoder(fillOrder, tileWidth, tileHeight).decodeT6(bdata, p, 0, tileHeight, t6Options);
      } catch (_) {}
    } else if (compression == COMPRESSION_ZIP) {
      List<int> data = p.toList(0, byteCount);
      List<int> outData = ZLibDecoder().decodeBytes(data);
