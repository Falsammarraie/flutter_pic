...
class WebPDecoder extends Decoder {
  ...
  Pic decodeFrame(int frame) {
    ...
    if (_info.hasAnimation) {
      ...
      InputBuffer frameData = _input.subset(f.frameSize, position: f.framePosition);

      return _decodeFrame(frameData, frame: frame);
    }

    if (_info.format == WebPInfo.FORMAT_LOSSLESS) {
      InputBuffer data = _input.subset(_info.vp8Size, position: _info.vp8Position);
  ...
  Pic decodeImage(List<int> bytes, {int frame = 0}) {
    startDecode(bytes);
    _info.frame = 0;
    _info.numFrames = 1;
  ...
  Pic _decodeFrame(InputBuffer input, {int frame = 0}) {
    ...
    if (webp.hasAnimation) {
      ...
      InputBuffer frameData = input.subset(f.frameSize, position: f.framePosition);
    ...
    while (!input.isEOS && !found) {
      ...
      switch (tag) {
        ...
        case 'VP8L':
          ...
          break;
        case 'ALPH':
          webp.alphaData = InputBuffer(input.buffer, bigEndian: input.bigEndian);
