...
/// file and using that decoder to decode the file into a single frame [Pic].
Pic decodeImage(List<int> data) {
...
/// containing one or more [Pic] frames.
...
/// [Pic] frames. See also [decodeAnimation].
...
/// Identify the format of the image using the file extension of the given
/// [name], and decode the given file [bytes] to a single frame [Pic]. See
/// also [decodeImage].
Pic decodeNamedImage(List<int> bytes, String name) {
  Decoder decoder = getDecoderForNamedImage(name);
  if (decoder == null) {
    return null;
  }
...
/// [Encoder].
List<int> encodeNamedImage(Pic image, String name) {
  String n = name.toLowerCase();
  if (n.endsWith('.jpg') || n.endsWith('.jpeg')) {
...
Pic readPng(List<int> bytes) => decodePng(bytes);

/// Encode an image to the PNG format.
List<int> encodePng(Pic image, {int level = 6}) {
...
Pic decodePsd(List<int> bytes) {
  return new PsdDecoder().decodeImage(bytes);
}

/// Decode an OpenEXR formatted image, tone-mapped using the
/// given [exposure] to a low-dynamic-range [Pic].
Pic decodeExr(List<int> bytes, {double exposure = 1.0}) {
