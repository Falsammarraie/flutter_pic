...
/// the width and height resolution of the animation canvas, and each [Pic]
...
/// the canvas prior to drawing the frame: [Pic.DISPOSE_NONE] indicates the
/// canvas should be left alone; [Pic.DISPOSE_CLEAR] indicates the canvas
/// should be cleared. For partial frame images,[Pic.DISPOSE_NONE] is used
...
abstract class Decoder {
  ...
  HdrImage decodeHdrImage(List<int> bytes, {int frame = 0}) {
    Pic img = decodeImage(bytes, frame: frame);
    if (img == null) {
      return null;
    }
  ...
  /// Non animated image files will only have [frame] 0. An [Pic]
