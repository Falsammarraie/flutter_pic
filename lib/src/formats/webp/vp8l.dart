...
class VP8L {
  ...
  Pic decode() {
    ...
    image = Pic(webp.width, webp.height);

    if (!_decodeImageData(_pixels, webp.width, webp.height, webp.height, _processRows)) {
      return null;
    }
    ...
    switch (type) {
      ...
      case VP8LTransform.CROSS_COLOR_TRANSFORM:
        ...
        transform.data = _decodeImageStream(
            _subSampleSize(transform.xsize, transform.bits), _subSampleSize(transform.ysize, transform.bits), false);
        break;
      case VP8LTransform.COLOR_INDEXING_TRANSFORM:
        final int numColors = br.readBits(8) + 1;
        final int bits = (numColors > 16) ? 0 : (numColors > 4) ? 1 : (numColors > 2) ? 2 : 3;
  ...
  bool _decodeImageData(dynamic data, int width, int height, int lastRow, dynamic processFunc) {
  ...
  void _applyInverseTransforms(int numRows, int rows) {
    ...
    while (n-- > 0) {
      ...
      transform.inverseTransform(startRow, endRow, _pixels, rowsIn, _pixels, rowsOut);
      rowsIn = rowsOut;
    }
  }
  ...
  HTreeGroup _getHtreeGroupForPos(int x, int y) {
    int metaIndex = _getMetaIndex(_huffmanImage, _huffmanXsize, _huffmanSubsampleBits, x, y);
...
class InternalVP8L extends VP8L {
  ...
  bool decodeImageData(dynamic data, int width, int height, int lastRow, dynamic processFunc) =>
      _decodeImageData(data, width, height, lastRow, processFunc);

  Uint32List decodeImageStream(int xsize, int ysize, bool isLevel0) => _decodeImageStream(xsize, ysize, isLevel0);

  bool allocateInternalBuffers32b() => _allocateInternalBuffers32b();

  bool allocateInternalBuffers8b() => _allocateInternalBuffers8b();

  bool decodeAlphaData(int width, int height, int lastRow) => _decodeAlphaData(width, height, lastRow);
