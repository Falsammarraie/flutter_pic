...
class VP8 {
  ...
  Pic decode() {
    ...
    output = Pic(webp.width, webp.height);
  ...
  bool _parsePartitions(InputBuffer input) {
    ...
    InputBuffer pin = input.subset(bufEnd - partStart, position: input.position + partStart);
      ...
      for (int b = 0; b < NUM_BANDS; ++b) {
        for (int c = 0; c < NUM_CTX; ++c) {
          for (int p = 0; p < NUM_PROBAS; ++p) {
            final int v = br.getBit(COEFFS_UPDATE_PROBA[t][b][c][p]) != 0 ? br.getValue(8) : COEFFS_PROBA_0[t][b][c][p];
  ...
  bool _initFrame() {
    ...
    _cacheY = InputBuffer(new Uint8List(16 * _cacheYStride + extra_y), offset: extra_y);

    _cacheU = InputBuffer(new Uint8List(8 * _cacheUVStride + extra_uv), offset: extra_uv);

    _cacheV = InputBuffer(new Uint8List(8 * _cacheUVStride + extra_uv), offset: extra_uv);
  ...
  void _upsample(InputBuffer topY, InputBuffer bottomY, InputBuffer topU, InputBuffer topV, InputBuffer curU,
      InputBuffer curV, InputBuffer topDst, InputBuffer bottomDst, int len) {
    ...
    for (int x = 1; x <= lastPixelPair; ++x) {
      ...
      _yuvToRgba(topY[2 * x - 1], uv0 & 0xff, (uv0 >> 16), new InputBuffer.from(topDst, offset: (2 * x - 1) * 4));

      _yuvToRgba(topY[2 * x - 0], uv1 & 0xff, (uv1 >> 16), new InputBuffer.from(topDst, offset: (2 * x - 0) * 4));
      ...
      if (bottomY != null) {
        ...
        _yuvToRgba(
            bottomY[2 * x - 1], uv0 & 0xff, (uv0 >> 16), new InputBuffer.from(bottomDst, offset: (2 * x - 1) * 4));

        _yuvToRgba(bottomY[2 * x], uv1 & 0xff, (uv1 >> 16), new InputBuffer.from(bottomDst, offset: (2 * x + 0) * 4));
      }
    ...
    if ((len & 1) == 0) {
      final int uv0 = (3 * tl_uv + l_uv + 0x00020002) >> 2;
      _yuvToRgba(topY[len - 1], uv0 & 0xff, (uv0 >> 16), new InputBuffer.from(topDst, offset: (len - 1) * 4));

      if (bottomY != null) {
        final int uv0 = (3 * l_uv + tl_uv + 0x00020002) >> 2;
        _yuvToRgba(bottomY[len - 1], uv0 & 0xff, (uv0 >> 16), new InputBuffer.from(bottomDst, offset: (len - 1) * 4));
      }
    }
  }
  ...
  int _emitFancyRGB(int mbY, int mbW, int mbH) {
    ...
    if (y == 0) {
      ...
      _upsample(_tmpY, curY, topU, topV, curU, curV, new InputBuffer.from(dst, offset: -stride), dst, mbW);
      ++numLinesOut;
    }
    ...
    for (; y + 2 < yEnd; y += 2) {
      ...
      _upsample(new InputBuffer.from(curY, offset: -_cacheYStride), curY, topU, topV, curU, curV,
    ...
    if (_cropTop + yEnd < _cropBottom) {
      ...
      if ((yEnd & 1) == 0) {
        _upsample(curY, null, curU, curV, curU, curV, new InputBuffer.from(dst, offset: stride), null, mbW);
      }
    }
  ...
  bool _decodeMB(VP8BitReader tokenBr) {
    ...
    if (_segmentHeader.updateMap) {
      ...
      _segment = br.getBit(_proba.segments[0]) == 0 ? br.getBit(_proba.segments[1]) : 2 + br.getBit(_proba.segments[2]);
    }
  ...
  static const List<int> kBands = const [0, 1, 2, 3, 6, 4, 5, 6, 6, 6, 6, 6, 6, 6, 6, 7, 0];
  ...
  static const List<int> kCat6 = const [254, 254, 243, 230, 196, 177, 153, 140, 133, 130, 129];
  static const List<List<int>> kCat3456 = const [kCat3, kCat4, kCat5, kCat6];
  static const List<int> kZigzag = const [0, 1, 4, 8, 5, 2, 3, 6, 9, 12, 13, 10, 7, 11, 14, 15];
  ...
  int _getCoeffs(VP8BitReader br, List<VP8BandProbas> prob, int ctx, List<int> dq, int n, InputBuffer out) {
  ...
  void _parseIntraMode() {
    ...
    if (!block.isIntra4x4) {
      ...
      final int ymode =
          br.getBit(156) != 0 ? (br.getBit(128) != 0 ? TM_PRED : H_PRED) : (br.getBit(163) != 0 ? V_PRED : DC_PRED);
    ...
    block.uvmode =
        br.getBit(142) == 0 ? DC_PRED : br.getBit(114) == 0 ? V_PRED : br.getBit(183) != 0 ? TM_PRED : H_PRED;
  }
