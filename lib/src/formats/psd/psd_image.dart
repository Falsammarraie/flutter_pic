...
class PsdImage extends DecodeInfo {
  ...
  void _blend(int ar, int ag, int ab, int aa, int br, int bg, int bb, int ba, int blendMode, double opacity,
  ...
  static int _blendPinLight(int bottom, int top) {
    return (top < 128) ? _blendDarken(bottom, 2 * top) : _blendLighten(bottom, 2 * (top - 128));
  }
  ...
  void _readImageResources() {
    ...
    while (!_imageResourceData.isEOS) {
      ...
      if (blockSignature == RESOURCE_BLOCK_SIGNATURE) {
        imageResources[blockId] = PsdImageResource(blockId, blockName, blockData);
      }
    }
  }
  ...
  void _readMergeImageData() {
    ...
    for (int i = 0; i < channels; ++i) {
      mergeImageChannels
          .add(new PsdChannel.read(_imageData, i == 3 ? -1 : i, width, height, depth, compression, lineLengths, i));
    }

    mergedImage = createImageFromChannels(colorMode, depth, width, height, mergeImageChannels);
  }
  ...
  static Pic createImageFromChannels(int colorMode, int bitDepth, int width, int height, List<PsdChannel> channelList) {
    Pic output = Pic(width, height);
