...
List<int> findTrim(Pic src, {TrimMode mode = TrimMode.transparent, Trim sides = Trim.all}) {
  int h = src.height;
  int w = src.width;
...
/// and [Trim.right].
Pic trim(Pic src, {TrimMode mode = TrimMode.transparent, Trim sides = Trim.all}) {
  if (mode == TrimMode.transparent && src.channels == Channels.rgb) {
    return new Pic.from(src);
  }

  List<int> crop = findTrim(src, mode: mode, sides: sides);

  Pic dst = Pic(crop[2], crop[3], channels: Channels.rgba, exif: src.exif, iccp: src.iccProfile);

  copyInto(dst, src, srcX: crop[0], srcY: crop[1], srcW: crop[2], srcH: crop[3], blend: false);
