...
Pic bumpToNormal(Pic src, {num strength = 2.0}) {
  Pic dest = Pic.from(src);

  for (var y = 0; y < src.height; ++y) {
    for (var x = 0; x < src.width; ++x) {
      final height = getRed(src.getPixel(x, y)) / 255.0;
      var du = (height - getRed(src.getPixel(x < src.width - 1 ? x + 1 : x, y)) / 255.0) * strength;
      var dv = (height - getRed(src.getPixel(x, y < src.height - 1 ? y + 1 : y)) / 255.0) * strength;
