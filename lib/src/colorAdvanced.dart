...
int getColor(int r, int g, int b, [int a = 255]) =>
    (clamp255(a) << 24) | (clamp255(b) << 16) | (clamp255(g) << 8) | (clamp255(r));
...
int getChannel(int color, Channel channel) => channel == Channel.red
    ...
    : channel == Channel.green
        ? getGreen(color)
        : channel == Channel.blue ? getBlue(color) : channel == Channel.alpha ? getAlpha(color) : getLuminance(color);
...
int setChannel(int color, Channel channel, int value) => channel == Channel.red
    ...
    : channel == Channel.green
        ...
        : channel == Channel.blue ? setBlue(color, value) : channel == Channel.alpha ? setAlpha(color, value) : color;
...
/// by [value].
int setGreen(int color, int value) => (color & 0xffff00ff) | (clamp255(value) << 8);
...
/// by [value].
int setAlpha(int color, int value) => (color & 0x00ffffff) | (clamp255(value) << 24);
...
/// Returns the luminance (grayscale) value of the color.
int getLuminanceRgb(int r, int g, int b) => (0.299 * r + 0.587 * g + 0.114 * b).round();
...
List<int> hsvToRgb(num hue, num saturation, num brightness) {
  ...
  switch (h.toInt()) {
    case 0:
      return [(brightness * 255.0).round(), (t * 255.0).round(), (p * 255.0).round()];
    case 1:
      return [(q * 255.0).round(), (brightness * 255.0).round(), (p * 255.0).round()];
    case 2:
      return [(p * 255.0).round(), (brightness * 255.0).round(), (t * 255.0).round()];
    case 3:
      return [(p * 255.0).round(), (q * 255.0).round(), (brightness * 255.0).round()];
    case 4:
      return [(t * 255.0).round(), (p * 255.0).round(), (brightness * 255.0).round()];
    case 5:
      return [(brightness * 255.0).round(), (p * 255.0).round(), (q * 255.0).round()];
    default:
...
List<int> xyzToRgb(num x, num y, num z) {
  ...
  return [(r * 255).clamp(0, 255).toInt(), (g * 255).clamp(0, 255).toInt(), (b * 255).clamp(0, 255).toInt()];
}
...
List<int> labToRgb(num l, num a, num b) {
  ...
  return [(R * 255.0).clamp(0, 255).toInt(), (G * 255.0).clamp(0, 255).toInt(), (B * 255.0).clamp(0, 255).toInt()];
}
