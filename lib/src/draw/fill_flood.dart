...
/// given [color].
Pic fillFlood(Pic src, int x, int y, int color, {num threshold = 0.0, bool compareAlpha = false}) {
  ...
  if (threshold > 0) {
    ...
    array = (int y, int x) {
      return visited[y * src.width + x] == 0 && _testPixelLabColorDistance(src, x, y, lab, threshold);
    };
  } else if (!compareAlpha) {
    array = (int y, int x) {
      return visited[y * src.width + x] == 0 && setAlpha(src.getPixel(x, y), 0) != srcColor;
    };
  } else {
    array = (int y, int x) {
...
/// image [src].
Uint8List maskFlood(Pic src, int x, int y, {num threshold = 0.0, bool compareAlpha = false, int fillValue = 255}) {
  var visited = Uint8List(src.width * src.height);
  ...
  _TestPixel array;
  if (threshold > 0) {
    List<num> lab = rgbToLab(getRed(srcColor), getGreen(srcColor), getBlue(srcColor));
    ...
    array = (int y, int x) {
      return visited[y * src.width + x] == 0 &&
          (ret[y * src.width + x] != 0 || _testPixelLabColorDistance(src, x, y, lab, threshold));
    };
  } else if (!compareAlpha) {
    array = (int y, int x) {
      return visited[y * src.width + x] == 0 &&
          (ret[y * src.width + x] != 0 || setAlpha(src.getPixel(x, y), 0) != srcColor);
    };
  } else {
    array = (int y, int x) {
      return visited[y * src.width + x] == 0 && (ret[y * src.width + x] != 0 || src.getPixel(x, y) != srcColor);
    };
  }
...
bool _testPixelLabColorDistance(Pic src, int x, int y, List<num> refColor, num threshold) {
  ...
  return ColorAdvanced.distance(pixelColor, refColor, compareAlpha) > threshold;
}

// Adam Milazzo (2015). A More Efficient Flood Fill.
// http://www.adammil.net/blog/v126_A_More_Efficient_Flood_Fill.html
void _fill4(Pic src, int x, int y, _TestPixel array, _MarkPixel mark, Uint8List visited) {
...
void _fill4Core(Pic src, int x, int y, _TestPixel array, _MarkPixel mark, Uint8List visited) {
  if (visited[y * src.width + x] == 1) {
    return;
  }
